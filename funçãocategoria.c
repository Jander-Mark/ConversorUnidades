...
bool verificarNumero(char str[30]) {
    return 0;
}
bool validarEntrada(double original, char *categoria) {
    return 0;
}
double converter(double original, double entrada, double saida) {
    return 0;
}
...
void realizarConversao(char categoria[]) {
    ...
    int numUnidades = sizeof(unidades) / sizeof(Unidade);
